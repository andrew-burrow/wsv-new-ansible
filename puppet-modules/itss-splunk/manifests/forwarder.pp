...
class splunk::forwarder (
  ...
  $addons                 = {},
  $splunk_uf_version      = $splunk::params::splunk_uf_version,
  $splunk_installed_ver   = $splunk::params::splunk_installed_ver,
) inherits splunk::params {
#Do nothing --- Dummy class
}
/*
class splunk::forwarder::decommissioned::Kyndryl::commisionthisifrequired (
  $server                 = $splunk::params::server,
  $package_source         = $splunk::params::forwarder_pkg_src,
  $package_name           = $splunk::params::forwarder_pkg_name,
  $package_ensure         = $splunk::params::forwarder_pkg_ensure,
 ...
 # $tag_resources = { tag => 'splunk_forwarder' }
  ...
  file { "${forwarder_confdir}/system/local/deploymentclient.conf":
    ...
    tag    => 'splunk_forwarder',
    source => 'puppet:///modules/splunk/deploymentclient.conf',
  }
  ...
  if ( $splunk_installed_ver != $splunk_uf_version ) {
      
      file { "${forwarder_confdir}/auth/ca.pem.orig":
    		 ensure => absent,
   			 tag    => 'splunk_forwarder',
 		 }
         
      file { "${forwarder_confdir}/auth/cacert.pem":
    		ensure => absent,
    		tag    => 'splunk_forwarder',
  		}
    }    
         
  fact { "splunk_forwarder_profile" :
            content => "${splunk_uf_version}",
            target  => "splunk.forwarder.profile",
            ensure  => present,
        }
