...
#ACCtionReplica ODBC DataSource
...
for ISname in common vwa tac ; do
	doMqsiExec "${MQSI_BIN_LOC}/mqsicreateexecutiongroup ${INTEGRATION_NODE_NAME} -e IS-${env}-${ISname}" ;
	doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} -e IS-${env}-${ISname} -o ComIbmJVMManager -n jvmSystemProperty -v\"-Dlog4j.configurationFile=/var/iib/${env}/config/eip-${ISname}-log4j2.xml -Deip.config=/var/iib/${env}/config/eip-config.xml\"" ;
...
doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} -e IS-${env}-common -o ExecutionGroup -n soapNodesUseEmbeddedListener,httpNodesUseEmbeddedListener -v true,true"
...
doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} -e IS-${env}-tac -o ExecutionGroup -n soapNodesUseEmbeddedListener,httpNodesUseEmbeddedListener -v true,true"
...
# Integration Server "tac"
doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} \
	-e IS-${env}-tac \
        -o HTTPConnector -n address,explicitlySetPortNumber -v ${INTEGRATION_NODE_IP},${INTEGRATION_SERVER_TAC_PORT}"

# Integration Server "vwa"
doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} \
	-e IS-${env}-vwa \
        -o HTTPConnector -n address,explicitlySetPortNumber -v ${INTEGRATION_NODE_IP},${INTEGRATION_SERVER_VWA_PORT}"
...
#doMqsiExec "${MQSI_BIN_LOC}/mqsireportproperties ${INTEGRATION_NODE_NAME} -c ODBCProviders -o ${AR_ODBC_PROVIDER_NAME} -r"
...
#
doMqsiExec "${MQSI_BIN_LOC}/mqsicreateconfigurableservice ${INTEGRATION_NODE_NAME} -c SMTP -o SMTP_VWA -n serverName -v ${SMTP_SERVER_VWA}"
