...
# load property file
#
PROPS=${CONFIGDIR}/IIBRAD_${env}_config.properties
if [ ! -f $PROPS ] ; then
...
#set global cache
#RAD broker does not require global cache
#doMqsiExec "${MQSI_BIN_LOC}/mqsichangebroker ${INTEGRATION_NODE_NAME} -b default"
		
#Start
doMqsiExec "${MQSI_BIN_LOC}/mqsistart ${INTEGRATION_NODE_NAME}"

						
#ACCtionReplica JDBC DataSource
#Database setup not required 
#doMqsiExec "${MQSI_BIN_LOC}/mqsisetdbparms ${INTEGRATION_NODE_NAME} -n ${AR_JDBC_SEC_NAME} -u ${AR_USER_NAME} -p ${AR_USER_PASSWORD}"
#doMqsiExec "${MQSI_BIN_LOC}/mqsicreateconfigurableservice ${INTEGRATION_NODE_NAME} -c JDBCProviders -o ${AR_JDBC_PROVIDER_NAME} -n connectionUrlFormat,connectionUrlFormatAttr1,connectionUrlFormatAttr2,connectionUrlFormatAttr3,connectionUrlFormatAttr4,connectionUrlFormatAttr5,databaseName,databaseSchemaNames,databaseType,databaseVersion,description,environmentParms,jarsURL,jdbcProviderXASupport,maxConnectionPoolSize,portNumber,securityIdentity,serverName,type4DatasourceClassName,type4DriverClassName -v ${AR_connectionUrlFormat},${AR_connectionUrlFormatAttr1},${AR_connectionUrlFormatAttr2},${AR_connectionUrlFormatAttr3},${AR_connectionUrlFormatAttr4},${AR_connectionUrlFormatAttr5},${AR_databaseName},${AR_databaseSchemaNames},${AR_databaseType},${AR_databaseVersion},${AR_description},${AR_environmentParms},${AR_jarsURL},${AR_jdbcProviderXASupport},${AR_maxConnectionPoolSize},${AR_portNumber},${AR_securityIdentity},${AR_serverName},${AR_type4DatasourceClassName},${AR_type4DriverClassName}"

#TEMPUS JDBC DataSource
#Database setup not required
#doMqsiExec "${MQSI_BIN_LOC}/mqsisetdbparms ${INTEGRATION_NODE_NAME} -n ${TPC2_JDBC_SEC_NAME} -u ${TPC2_USER_NAME} -p ${TPC2_USER_PASSWORD}"
#doMqsiExec "${MQSI_BIN_LOC}/mqsicreateconfigurableservice ${INTEGRATION_NODE_NAME} -c JDBCProviders -o ${TPC2_JDBC_PROVIDER_NAME} -n connectionUrlFormat,connectionUrlFormatAttr1,connectionUrlFormatAttr2,connectionUrlFormatAttr3,connectionUrlFormatAttr4,connectionUrlFormatAttr5,databaseName,databaseSchemaNames,databaseType,databaseVersion,description,environmentParms,jarsURL,jdbcProviderXASupport,maxConnectionPoolSize,portNumber,securityIdentity,serverName,type4DatasourceClassName,type4DriverClassName -v ${TPC2_connectionUrlFormat},${TPC2_connectionUrlFormatAttr1},${TPC2_connectionUrlFormatAttr2},${TPC2_connectionUrlFormatAttr3},${TPC2_connectionUrlFormatAttr4},${TPC2_connectionUrlFormatAttr5},${TPC2_databaseName},${TPC2_databaseSchemaNames},${TPC2_databaseType},${TPC2_databaseVersion},${TPC2_description},${TPC2_environmentParms},${TPC2_jarsURL},${TPC2_jdbcProviderXASupport},${TPC2_maxConnectionPoolSize},${TPC2_portNumber},${TPC2_securityIdentity},${TPC2_serverName},${TPC2_type4DatasourceClassName},${TPC2_type4DriverClassName}"

#create Integration Servers
doMqsiExec "${MQSI_BIN_LOC}/mqsicreateexecutiongroup ${INTEGRATION_NODE_NAME} -e IS-${env}-rad"		
#doMqsiExec "${MQSI_BIN_LOC}/mqsicreateexecutiongroup ${INTEGRATION_NODE_NAME} -e IS-${env}-vwa"
#doMqsiExec "${MQSI_BIN_LOC}/mqsicreateexecutiongroup ${INTEGRATION_NODE_NAME} -e IS-${env}-tac"
...
#disable soap node listener. forces integration server to use server level http listener
doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} -e IS-${env}-rad -o ExecutionGroup -n soapNodesUseEmbeddedListener -v false"
#doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} -e IS-${env}-vwa -o ExecutionGroup -n soapNodesUseEmbeddedListener -v false"
#doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} -e IS-${env}-tac -o ExecutionGroup -n soapNodesUseEmbeddedListener -v false"
		
######################################################################
#Set HTTP IP and Port for Integration Server level SOAP listeners
######################################################################
# Integration Server "common"
doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} \
	-e IS-${env}-rad \
        -o HTTPConnector -n address -v ${INTEGRATION_NODE_IP}"
doMqsiExec "${MQSI_BIN_LOC}/mqsichangeproperties ${INTEGRATION_NODE_NAME} \
	-e IS-${env}-rad \
        -o HTTPConnector -n port -v ${INTEGRATION_SERVER_RAD_PORT}"
# -----------------------------------------------------------------------#
...
runmqckm -certreq -create -db ${IIBSSLDIR}/webadmin.jks \
	...
	-label ${INTEGRATION_NODE_NAME} \
        -dn "CN=${INTEGRATION_NODE_IP},OU=${ENV},OU=IBM IIB,O=ITSS,L=Melbourne,ST=VIC,C=AU" \
...
#doMqsiExec "${MQSI_BIN_LOC}/mqsireportproperties ${INTEGRATION_NODE_NAME} -c JDBCProviders -o ${AR_JDBC_PROVIDER_NAME} -r"
#doMqsiExec "${MQSI_BIN_LOC}/mqsireportproperties ${INTEGRATION_NODE_NAME} -c JDBCProviders -o ${TPC2_JDBC_PROVIDER_NAME} -r"
...
# -----------------------------------------------------------------------#
# Create TCP IP CLIENT configurable service 
#

doMqsiExec "${MQSI_BIN_LOC}/mqsicreateconfigurableservice ${INTEGRATION_NODE_NAME} -c TCPIPClient -o HICAPS -n Port,Hostname -v ${TCP_IP_CLIENT_PORT},${TCP_IP_CLIENT_HOST}
