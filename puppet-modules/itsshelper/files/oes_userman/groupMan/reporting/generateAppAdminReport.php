		...
		for ($x=0; $x<$appPageInfo[$i]["auvwaapplicationrole"]["count"]; $x++) {
			...
			for ($z=0; $z<$appRoleInfo[$y]["member"]["count"]; $z++) {
			  ...
			  if (!(array_key_exists($tmpUserDN, $allUserInfo))) {
			      ...
			      $tmpUser = array("dn" => $tmpUserDN, "cn" => substr($tmpUserCN[1],0,-3), "company" => substr($tmpUserCompany[0],7), "loginDisabled" => $tmpUserInfo[0]["logindisabled"][0], "loginTime" => $logintime, "createTime" => $createtime, "mail" => $tmpUserInfo[0]["mail"][0], "AUVWAOANLicenseNumber" => $tmpUserInfo[0]["auvwaoanlicensenumber"][0], "AUVWAshortName" => $tmpUserInfo[0]["auvwashortname"][0]);
			...
			$allAppInfoTmp = array("appName" => $appPageInfo[$i]["description"][0], "appRole" => $appRoleInfo[$y]["description"][0], "appUser" => $allUserInfo[$tmpUserDN]["cn"], "userCompany" => $allUserInfo[$tmpUserDN]["company"], "loginDisabled" => $allUserInfo[$tmpUserDN]["loginDisabled"], "loginTime" => $allUserInfo[$tmpUserDN]["loginTime"], "createTime" => $allUserInfo[$tmpUserDN]["createTime"],"mail" => $allUserInfo[$tmpUserDN]["mail"], "AUVWAOANLicenseNumber" => $allUserInfo[$tmpUserDN]["AUVWAOANLicenseNumber"], "AUVWAshortName" => $allUserInfo[$tmpUserDN]["AUVWAshortName"]);
...
function getUserInfo($UserDN) {
	...
	$basedn = $UserDN;
	$attributes = array("cn", "auvwausercompanylink", "logintime","logindisabled","AUVWAshortName","createTimestamp","mail","AUVWAOANLicenseNumber");
...
function generateXSLReport($appInfo, $adminDN) {
	...
	$worksheet->write(0, 4, 'Email',$format_header);
	$worksheet->write(0, 5, 'Company',$format_header);
	$worksheet->write(0, 6, 'Last Login',$format_header);
        $worksheet->write(0, 7, 'Create Date',$format_header);
        $worksheet->write(0, 8, 'Login Disabled',$format_header);
	$worksheet->write(0, 9, 'OANLicenseNumber',$format_header);
	$worksheet->write(0, 10, 'Changes Needed',$format_header);
	...
	while (list($key, $value) = each($appInfo)) {
	    ...
	    $worksheet->write($row, 2, $value["appUser"], $format_row_left);
            ...
            $worksheet->write($row, 4, $value["mail"], $format_row_left);
	    $worksheet->write($row, 5, $value["userCompany"], $format_row_left);
	    $worksheet->write($row, 6, $value["loginTime"], $format_row_left);
	    $worksheet->write($row, 7, $value["createTime"], $format_row_left);
            $worksheet->write($row, 8, $value["loginDisabled"], $format_row_left);
            $worksheet->write($row, 9, $value["AUVWAOANLicenseNumber"], $format_row_left);
